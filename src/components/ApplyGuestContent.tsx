...
export const ApplyGuestContent = () => {
  ...
  const validate = (s: number): boolean => {
    ...
    if (s === 1) {
      ...
      else if (!/^[a-zA-Z\s'-]+$/.test(form.firstName.trim())) e.firstName = "Letters only please";

      if (!form.email.trim()) e.email = "Email is needed";
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) e.email = "Valid email please";

      if (!form.phone.trim()) e.phone = "Phone number helps";
      else if (!/^\+?[\d\s()-]+$/.test(form.phone.trim())) e.phone = "Digits only please";
      else if (form.phone.replace(/\D/g, "").length < 10 || form.phone.replace(/\D/g, "").length > 15)
        e.phone = "Valid phone please";

      if (form.instagram.trim() && !/^@?[a-zA-Z0-9._]{1,30}$/.test(form.instagram.trim()))
        e.instagram = "Valid Instagram handle please";

      if (form.linkedin.trim() && !/^(https?:\/\/)?(www\.)?linkedin\.com\/.+/i.test(form.linkedin.trim()))
        e.linkedin = "Valid LinkedIn URL please";
    }
                ...
                <motion.div key={step}
                  ...
                  {step === 1 && (
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <Field label="Instagram" error={errors.instagram} icon={<Instagram className="w-4 h-4" />}>
                          <input className={inputCls(errors.instagram)} placeholder="@rahulmehta"
                            value={form.instagram} onChange={e => set("instagram", e.target.value)} />
                        </Field>
                        <Field label="LinkedIn" error={errors.linkedin} icon={<Linkedin className="w-4 h-4" />}>
                          <input className={inputCls(errors.linkedin)} placeholder="linkedin.com/in/..."
        ...
        <section className="mb-24 pb-10">
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            ...
            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-xl bg-white/5 border border-white/10">
              ...
              <a href="mailto:Girish.singaria@thefoundershow.com"
                className="text-[#e8c97e] text-sm font-medium hover:underline">
                Girish.singaria@thefoundershow.com
